...
type VSwitchService struct {
	...
	// Set wraps functionality of the 'ovs-vsctl remove' subcommand.
	Remove *VSwitchRemoveService

	// Set wraps functionality of the 'ovs-vsctl add' subcommand.
	Add *VSwitchAddService
...
func (v *VSwitchGetService) PortTrunkVlans(portName string) ([]uint16, error) {
	vlans := []uint16{}
	args := []string{"get", "port", portName, "trunks"}
	out, err := v.v.exec(args...)
	if err != nil {
		return nil, err
	}
	if err := json.Unmarshal(out, &vlans); err != nil {
		return nil, err
	}
	return vlans, nil
}
...
// A VSwitchRemoveService is used in a VSwitchService to execute 'ovs-vsctl remove'
// subcommands.
type VSwitchRemoveService struct {
	// v provides the required exec method.
	v *VSwitchService
}

func (v *VSwitchRemoveService) RemoveVlanFromTrunk(trunkPort string, vlan uint16) error {
	_, err := v.v.exec("remove", "port", trunkPort, "trunk", strconv.Itoa(int(vlan)))
	return err
}

// A VSwitchAddService is used in a VSwitchService to execute 'ovs-vsctl add'
// subcommands.
type VSwitchAddService struct {
	// v provides the required exec method.
	v *VSwitchService
}

func (v *VSwitchAddService) AddVlanToTrunk(trunkPort string, vlan uint16) error {
	_, err := v.v.exec("add", "port", trunkPort, "trunk", strconv.Itoa(int(vlan)))
	return err
}
